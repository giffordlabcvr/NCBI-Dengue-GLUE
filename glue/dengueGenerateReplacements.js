// Declare list of serotypes 
var serotypes = [ 1, 2, 3, 4 ];

// Note: here we assume that all coding features defined in the project are defined in each serotype master reference  
var featuresList = glue.tableToObjects(glue.command(["list", "feature", "-w", "featureMetatags.name = 'CODES_AMINO_ACIDS' and featureMetatags.value = true", "name", "displayName", "parent.name"]));


// Iterate through serotypes
for(var i = 0; i < serotypes.length; i++) {

	var serotype = serotypes[i];
	//glue.log("INFO", "serotype RESULT WAS ", serotype);

	var comparisonRefName = "REF_MASTER_DENV"+serotype;
	var sourceName = "fasta-refseqs-yale-denv"+serotype;
	...
	var whereClause = "sequence.source.name = '"+sourceName+"'";
									...
									_.each(refAas, function(refAa) {
										...
										_.each(memberAas, function(memberAa) {
											...
											if(refAa != memberAa) {
												...
												var replacementID = "denv"+serotype+":"+featureObj.name+":"+refAa+":"+memberAaObj.codonLabel+":"+memberAa;
			...
			if(!variationExists) {
				...
				glue.inMode("custom-table-row/denv_replacement/"+replacementObj.id, function() {
					...
					glue.command(["set", "link-target", "variation", 
						"reference/"+comparisonRefName+"/feature-location/"+replacementObj.feature+
